# 🚀 Memory Cleaner v0.3.0

**Windows 전용 스마트 메모리 정리 도구** — Tauri 2 + React 18 + Rust

[![Version](https://img.shields.io/badge/version-0.3.0-blue)](https://github.com/Sangmoo/MemoryCleaner/releases)
...
| 기능 | 설명 |
|------|------|
| 설정 프리셋 (v0.3.0) | 🎮 게임 · 💼 업무 · 🔋 절전 모드 원클릭 전환 |
| 메모리 경고 알림 | RAM 임계값 도달 시 OS 트레이 알림 (히스테리시스) |
| 보호 프로세스 | 자동·수동 정리에서 제외할 프로세스 지정 |
| 보호 목록 즉시 추가 (v0.3.0) | 프로세스 행 우클릭 → 보호 목록에 추가 |
| 설정 내보내기/가져오기 | JSON 파일로 백업 및 복원 |
| CPU 급등 감지 설정 (v0.3.0) | 설정에서 ON/OFF 가능 |

### 🔧 프로세스 제어 (v0.3.0 신규)
| 기능 | 설명 |
|------|------|
| 우선순위 조절 | 우클릭 → CPU 낮춤 / 유휴 / 정상화 (SetPriorityClass) |
| 보호 추가 | 우클릭 컨텍스트 메뉴에서 즉시 보호 목록 등록 |

### 📁 기타
| 기능 | 설명 |
|------|------|
| Kill 히스토리 | 종료 기록 저장, CSV 내보내기 (v0.3.0) |
| 디스크 정리 | TEMP, 브라우저 캐시, WU 캐시, 휴지통 정리 |
| 시작 프로그램 관리 | 레지스트리 시작 프로그램 ON/OFF |
| 동적 트레이 아이콘 | RAM 사용률에 따라 🟢🟡🔴 색상 변경 |
| 온보딩 투어 (v0.3.0) | 최초 실행 시 4단계 기능 소개 |
| 업데이트 확인 (v0.3.0) | GitHub Releases 최신 버전 자동 감지 |
| 다크/라이트 모드 | 전환 가능 |

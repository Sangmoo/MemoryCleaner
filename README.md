...
- **자동 새로고침** — 설정한 주기마다 목록 자동 갱신, 툴바에 카운트다운 표시
...
- **좌클릭** — 창 표시/숨기기 토글
- **우클릭** — 메뉴 (앱 열기 / 종료)
- **툴팁** — `Memory Cleaner · RAM 74%` 형식으로 5초마다 실시간 갱신
- 헤더에 자동 정리 ON 뱃지 표시
...
│  ├─ ProcessTable (가상 스크롤, 검색, 정렬, 그룹화)      │
│  ├─ ProcessDetailModal  ← 프로세스 상세 팝업           │
│  ├─ HistoryPanel / StartupPanel                       │
│  └─ SettingsModal (자동정리·제외시간대·자동시작·새로고침 간격·보호목록) │
└─────────────────────┬─────────────────────────────────┘
...
│  └─ lib.rs       — Tauri 빌더, 트레이(실시간 RAM%), 자동정리 루프 │
└───────────────────────────────────────────────────────┘
```
...
│   │   ├── ProcessTable.tsx          ← 검색·정렬·그룹화·CPU·보호 뱃지·상세 버튼
│   │   ├── ProcessDetailModal.tsx    ← 프로세스 상세 팝업
│   │   ├── HistoryPanel.tsx          ← Kill 히스토리
│   │   ├── StartupPanel.tsx          ← 시작 프로그램 관리
│   │   ├── SettingsModal.tsx         ← 자동정리·제외시간대·자동시작·새로고침 간격·보호목록
...
│   ├── src/
│   │   ├── main.rs
│   │   ├── lib.rs                    ← 트레이(ID·실시간 툴팁) + 자동정리 루프
│   │   ├── commands.rs
│   │   ├── settings.rs               ← process_refresh_seconds 포함
│   │   ├── history.rs
│   │   └── startup.rs
│   ├── icons/
│   ├── Cargo.toml
...
- **앱 설정**
  - Windows 시작 시 자동 실행 ON/OFF (레지스트리 Run 키)
  - 프로세스 새로고침 간격 선택 (5초 / 10초 / 30초 / 1분 / 3분 / 5분)
...
| 버전 | 내용 |
|------|------|
| **v0.2.0** | Tauri 2 + React 18 + Rust 재구성, 가상 스크롤, 실시간 그래프, CPU 컬럼, 검색/정렬, 프로세스 그룹화, 자동 새로고침(간격 설정), 자동 정리, 보호 프로세스, Kill 히스토리, 시작 프로그램 관리, 메모리 압축(EmptyWorkingSet), 임시 파일 정리, 시스템 트레이(실시간 RAM% 툴팁), OS 알림, 앱 자동시작, 프로세스 상세 팝업, 자동정리 제외 시간대 |
